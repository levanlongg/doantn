...
import { ContactService } from '../Ad-Service/contact.service';
...
export class ContactComponent implements OnInit {
  ...
  constructor(
    private category: ContactService,
  ...
  createEror() {
    this.toastr.error('Chưa thêm liên hệ', 'Thông báo!', { timeOut: 2000 });
  }
  updateSuccess() {
  ...
  deleteEror() {
    this.toastr.error('Chưa xóa liên hệ', 'Thông báo!', { timeOut: 2000 });
  }

  loadData() {
    this.category.getList().subscribe((res: any) => {
      this.items = res;
  ...
  SaveForm(values: any) {
    if (this.checkedid == 0) {
      this.category.postItem(values).subscribe((res) => {
        if (res) {
          var result = confirm("Bạn muốn thêm liên hệ?");
          if (result == true) {
            this.loadData();
            this.Create.hide();
            this.createSuccess();
  ...
  UpdateForm(values: any) {
    if (this.id = values.id) {
      this.category.editItem(this.id, values).subscribe((res) => {
        var result = confirm("Bạn muốn cập nhật liên hệ?");
        if (result == true) {
          this.loadData();
          this.Update.hide();
          this.updateSuccess();
